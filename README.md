...
---------------------------|:-----------------------------------------------------------
`:amount`                  | Сумма заказа
`:state`                   | Состояние заказа. (см. [таблицу сосотяний](#states))
...
<a name="states"></a>**Возможные состояния:**

 Состояние               | Описание
-------------------------|:-----------------------------------------------------------
`:paid`                  | Заказ оплачен. Уведомление об оплате магазину доставлено.
`:in_process`            | Заказ находится в обработке. Точный статус оплаты заказа определить невозможно. (к примеру, если пользователя отправило на InProgress URL, но уведомления на Pay URL от шлюза не поступало)
